...
namespace MindfulNeighborhood
{
    internal static class AssetUtils
    {
        ...
        private static IEnumerator GeneratePrefabAsync(string prefabName, Action<GameObject> onComplete)
        {
            ...
            switch (prefabName)
            {
                case "STR_CoastalHouseCFirePlace":
                    meshPath = "Assets/ArtAssets/Env/Structures/STR_CoastalHouseC/STR_CoastalHouseCFirePlace.fbx";
                    materialPath = "Assets/ArtAssets/Materials/Unique/STR_StoneFirePlace_A01.mat";
                    break;



                default:
                    Log.Warning($"[AssetUtils] Unknown prefab name: {prefabName}");
                    onComplete?.Invoke(null);
                    yield break;
            }

            // Load mesh
            Log.Msg($"[AssetUtils] Loading mesh: {meshPath}");
            ...
            {
                Log.Error($"[AssetUtils] Failed to load mesh: {meshPath}");
                if (meshHandle.OperationException != null)
                    Log.Error($"[AssetUtils] Exception: {meshHandle.OperationException}");
                onComplete?.Invoke(null);
                yield break;
            }
            ...
            if (loadedMesh == null)
            {
                Log.Error($"[AssetUtils] Loaded mesh is null: {meshPath}");
                onComplete?.Invoke(null);
                yield break;
            }

            // Load material
            Log.Msg($"[AssetUtils] Loading material: {materialPath}");
            ...
            {
                Log.Error($"[AssetUtils] Failed to load material: {materialPath}");
                if (materialHandle.OperationException != null)
                    Log.Error($"[AssetUtils] Exception: {materialHandle.OperationException}");
            ...
            {
                Log.Error($"[AssetUtils] Loaded material is null: {materialPath}");
                Addressables.Release(meshHandle);
                onComplete?.Invoke(null);
                yield break;
            }
            ...
            try
            {
                ...
                if (mf == null)
                {
                    Log.Error($"[AssetUtils] Failed to create MeshFilter for {prefabName}");
                ...
                Log.Msg($"[AssetUtils] Successfully created prefab: {prefabName}");
                cachedPrefabs[prefabName] = go;
                onComplete?.Invoke(go);
            }
            catch (System.Exception ex)
            {
                Log.Error($"[AssetUtils] Exception creating prefab {prefabName}: {ex.Message}");
                Log.Error($"[AssetUtils] Stack trace: {ex.StackTrace}");
                if (go != null)
                {
                    UnityEngine.Object.Destroy(go);
                }
